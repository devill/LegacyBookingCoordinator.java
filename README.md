...
@Test
public void bookFlightShouldCreateBookingSuccessfully() {
    factory.setOne(BookingRepository.class, new ParrotBookingRepository("💾"));
...
### 🔗 Alternative: Factory Builder Pattern
...
public void bookFlightShouldCreateBookingSuccessfully() {
    factory
        .substitute(BookingRepository.class, new ParrotBookingRepository("💾"))
        .substitute(FlightAvailabilityService.class, new ParrotFlightService("✈️"))
        .substitute(PartnerNotifier.class, new ParrotPartnerNotifier("📣"));
...
Transform your single test into a comprehensive test suite that covers multiple booking scenarios using JUnit's `@ParameterizedTest`. Instead of writing multiple similar tests, you'll define the system under test once and create approved files for each scenario.

### 🏆 Concept: Parameterized Approval Tests

Use JUnit's parameterized tests with approval testing:

```java
@ParameterizedTest
@ValueSource(strings = {"StandardBooking", "NoAvailability", "PremiumBooking"})
public void bookFlightMultipleScenarios(String scenario) {
    ...
    Approvals.verify(result, new ApprovalNamer() {
        @Override
        public String getApprovalName() {
            return "BookingCoordinatorTest.bookFlightMultipleScenarios." + scenario;
        }

        @Override
        public String getSourceFilePath() {
            return "src/test/java/com/legacybooking/BookingCoordinatorTest.java";
        }
    });
}
```

Create files like `BookingCoordinatorTest.bookFlightMultipleScenarios.StandardBooking.approved.txt` and `BookingCoordinatorTest.bookFlightMultipleScenarios.NoAvailability.approved.txt`. Each file runs as its own test automatically.

If you need to inject test values, you can create a test data class or use method parameters to vary the test inputs for different scenarios.
